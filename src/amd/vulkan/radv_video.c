...
static unsigned calc_dpb_size(struct radv_video_session *vid)
{
   ...
   case VK_VIDEO_CODEC_OPERATION_DECODE_H265_BIT_EXT: {
      ...
      width = align(width, 16);
      height = align(height, 16);
      if (vid->h265.profile_idc == STD_VIDEO_H265_PROFILE_IDC_MAIN_10)
                           ...
                           VkVideoSessionKHR *pVideoSession)
{
   ...
   case VK_VIDEO_CODEC_OPERATION_DECODE_H264_BIT_EXT:
      ...
      vid->h264.level = u_get_h264_level(pCreateInfo->maxCodedExtent.width,
                                         pCreateInfo->maxCodedExtent.height, &max_ref);
      break;
   case VK_VIDEO_CODEC_OPERATION_DECODE_H265_BIT_EXT:
      const struct VkVideoDecodeH265ProfileEXT *h265_profile =
         vk_find_struct_const(pCreateInfo->pVideoProfile->pNext, VIDEO_DECODE_H265_PROFILE_EXT);
   ...
   vid->db_alignment = (device->physical_device->rad_info.family >= CHIP_RENOIR &&
                        vid->max_coded.width > 32 &&
                        (vid->stream_type == RDECODE_CODEC_H265 &&
                         vid->h265.profile_idc == STD_VIDEO_H265_PROFILE_IDC_MAIN_10)) ? 64 : 32;
                                          ...
                                          VkVideoGetMemoryPropertiesKHR *pVideoSessionMemoryRequirements)
{
   ...
   *pVideoSessionMemoryRequirementsCount = num_memory_reqs;
   ...
   if (vid->stream_type == RDECODE_CODEC_H265) {
      ...
      pVideoSessionMemoryRequirements[idx].pMemoryRequirements->memoryRequirements.size = align(ctx_size, 4096);
      pVideoSessionMemoryRequirements[idx].pMemoryRequirements->memoryRequirements.alignment = 0;
      pVideoSessionMemoryRequirements[idx].pMemoryRequirements->memoryRequirements.memoryTypeBits = memory_type_bits;
   }
   return VK_SUCCESS;
                                            ...
                                            void *it_ptr)
{
   ...
   if (h265_pic_info->pStdPictureInfo->flags.short_term_ref_pic_set_sps_flag) {
      ...
      result.st_rps_bits = h265_pic_info->pStdPictureInfo->num_short_term_ref_pic_sets;
   }
                                    ...
                                    const struct VkVideoDecodeInfoKHR *frame_info)
{
   ...
   if (device->physical_device->rad_info.family >= CHIP_SIENNA_CICHLID &&
       (vid->stream_type == RDECODE_CODEC_H265 &&
        vid->h265.profile_idc == STD_VIDEO_H265_PROFILE_IDC_MAIN_10))
      decode->db_aligned_height = align(frame_info->codedExtent.height, 64);
