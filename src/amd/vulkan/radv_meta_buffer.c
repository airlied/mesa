    ...
    uint32_t                                    data)
{
	...
	if (cmd_buffer->queue_family_index == RADV_QUEUE_TRANSFER) {
		radv_transfer_cmd_fill_buffer(cmd_buffer, dst_buffer, dstOffset, fillSize, data);
		return;
	}
	...
	const VkBufferCopy*                         pRegions)
{
	...
	if (cmd_buffer->queue_family_index == RADV_QUEUE_TRANSFER) {
		radv_transfer_cmd_copy_buffer(cmd_buffer, src_buffer, dest_buffer, regionCount, pRegions);
		return;
	}
	...
	const void*                                 pData)
{
	...
	if (cmd_buffer->queue_family_index == RADV_QUEUE_TRANSFER) {
		radv_transfer_cmd_update_buffer(cmd_buffer, dst_buffer,
